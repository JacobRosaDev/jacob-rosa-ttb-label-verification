...
1. Stack: Python 3.12 + FastAPI backend, React (or plain HTML/JS) frontend, a vision 
model for extraction, no database (stateless / in-memory). Deploy target: a free-tier host (e.g. 
Railway, or Vercel + Render).  

2. Hard requirements that override convenience: single-label result 
...
3. Working cadence: when I say PLAN, propose an approach and list files/risks but 
